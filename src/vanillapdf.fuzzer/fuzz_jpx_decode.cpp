...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    HandleGuard<JPXDecodeFilterHandle, JPXDecodeFilter_Release> filter;
    if (JPXDecodeFilter_Create(filter.out()) != VANILLAPDF_ERROR_SUCCESS) {
        return 0;
    }

    HandleGuard<BufferHandle, Buffer_Release> output;
    JPXDecodeFilter_Decode(filter, input, output.out());

...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
    ...
    HandleGuard<ASCIIHexDecodeFilterHandle, ASCIIHexDecodeFilter_Release> filter;
    if (ASCIIHexDecodeFilter_Create(filter.out()) != VANILLAPDF_ERROR_SUCCESS) {
        return 0;
    }

    HandleGuard<BufferHandle, Buffer_Release> output;
    ASCIIHexDecodeFilter_Decode(filter, input, output.out());

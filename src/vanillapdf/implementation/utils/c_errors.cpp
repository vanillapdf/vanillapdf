...
#define CHECK_CODE(code, constant) \
    ...
    static bool GetValueName(error_type code, string_type& data, size_type& length) {
        ...
        CHECK_CODE(code, VANILLAPDF_ERROR_LICENSE_REQUIRED);
        CHECK_CODE(code, VANILLAPDF_ERROR_INSUFFICIENT_SPACE);
        CHECK_CODE(code, VANILLAPDF_ERROR_DATA_CORRUPTION);
        CHECK_CODE(code, VANILLAPDF_ERROR_IO);
        CHECK_CODE(code, VANILLAPDF_ERROR_CRYPTO);
        CHECK_CODE(code, VANILLAPDF_ERROR_IMAGE_CODEC);
        ...
        CHECK_CODE(code, VANILLAPDF_ERROR_PARSE_EXCEPTION);
        CHECK_CODE(code, VANILLAPDF_ERROR_INVALID_PASSWORD);
        CHECK_CODE(code, VANILLAPDF_ERROR_DUPLICATE_KEY);

        // Semantics
        CHECK_CODE(code, VANILLAPDF_ERROR_OPTIONAL_ENTRY_MISSING);
        CHECK_CODE(code, VANILLAPDF_ERROR_SEMANTIC_CONTEXT);
...
VANILLAPDF_API error_type CALLING_CONVENTION Errors_GetPrintableErrorText(error_type value, char* data, size_type size) {
    ...
    if (!success || value_data == nullptr || value_data_size == 0) {
        spdlog::error("Errors_GetPrintableErrorText: Failed to get error code");
        return VANILLAPDF_ERROR_GENERAL;
    }

    if (size < value_data_size) {
        spdlog::error("Errors_GetPrintableErrorText: Buffer too small");
        return VANILLAPDF_ERROR_GENERAL;
    }

    spdlog::debug("Errors_GetPrintableErrorText ({}): {}", value, value_data);

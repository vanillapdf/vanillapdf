...
void DocumentSigner::OnBeforeOutputFlush(IInputOutputStreamPtr output) {
    ...
    if (byte_ranges_offset == constant::BAD_OFFSET) {
        LOG_ERROR_AND_THROW(CryptoErrorException,"Invalid offset for byte range array: {:d}", byte_ranges_offset);
    }

    if (signature_contents_offset == constant::BAD_OFFSET) {
        LOG_ERROR_AND_THROW(CryptoErrorException,"Invalid offset for signature contents: {:d}", signature_contents_offset);
    }
    ...
    if (signature_encoded.size() > signature_contents_overriden_value.size()) {
        LOG_ERROR_AND_THROW(CryptoErrorException,"Pre-allocated signature size is not sufficient");
    }

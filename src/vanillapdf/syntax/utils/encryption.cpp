...
BufferPtr EncryptionUtils::ComputeObjectKey(
    ...
    if (final_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not finalize MD5 digest: {}", openssl_error);
    }
...
BufferPtr EncryptionUtils::ComputeRC4(const Buffer& key, types::size_type key_length, const Buffer& data) {
    ...
    if (init_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not initialize RC4 digest: {}", openssl_error);
    }
    ...
    if (set_key_length_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not set RC4 key length: {}", openssl_error);
    }

    auto init_result2 = EVP_EncryptInit(evp_cipher_ctx, nullptr, (unsigned char*)key.data(), nullptr);
    if (init_result2 != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not re-initialize RC4 digest: {}", openssl_error);
    }
    ...
    if (update_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not update RC4 digest: {}", openssl_error);
    }
    ...
    if (final_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not finalize RC4 digest: {}", openssl_error);
    }
...
BufferPtr EncryptionUtils::AESDecrypt(const Buffer& key, types::size_type key_length, const Buffer& data) {
    ...
    if (init_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not initialize AES digest: {}", openssl_error);
    }
    ...
    if (padding_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not disable AES decryption padding: {}", openssl_error);
    }
    ...
    if (update_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not decrypt update AES cipher: {}", openssl_error);
    }
    ...
    if (final_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not decrypt finalize AES cipher: {}", openssl_error);
    }
...
BufferPtr EncryptionUtils::AESEncrypt(const Buffer& key, types::size_type key_length, const Buffer& data) {
    ...
    if (rand_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not generate initialization vector for AES: {}", openssl_error);
    }
    ...
    if (init_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not initialize AES cipher: {}", openssl_error);
    }
    ...
    if (update_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not encrypt update AES cipher: {}", openssl_error);
    }
    ...
    if (final_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not encrypt finalize AES cipher: {}", openssl_error);
    }
...
BufferPtr EncryptionUtils::CalculateDecryptionCompareDataV3(
    ...
    for (int i = 0; i < 50; ++i) {
        ...
        if (init_result != 1) {
            auto openssl_error = CryptoUtils::GetLastOpensslError();
            LOG_ERROR_AND_THROW(CryptoErrorException, "Could not initialize MD5 cipher: {}", openssl_error);
        }

        auto update_result = EVP_DigestUpdate(evp_md_ctx, decryption_key_digest->data(), decryption_key_length);
        if (update_result != 1) {
            auto openssl_error = CryptoUtils::GetLastOpensslError();
            LOG_ERROR_AND_THROW(CryptoErrorException, "Could not update MD5 cipher: {}", openssl_error);
        }
        ...
        if (final_result != 1) {
            auto openssl_error = CryptoUtils::GetLastOpensslError();
            LOG_ERROR_AND_THROW(CryptoErrorException, "Could not finalize MD5 cipher: {}", openssl_error);
        }
...
(const syntax::ArrayObject<syntax::StringObjectPtr>& enveloped_data,
    ...
    if (init_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not initialize SHA digest: {}", openssl_error);
    }
    ...
    if (update_key_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not update SHA digest: {}", openssl_error);
    }
    ...
    if (final_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not finalize SHA digest: {}", openssl_error);
    }
...
BufferPtr EncryptionUtils::GenerateRandomData(int length) {
    ...
    if (rand_result != 1) {
        auto openssl_error = CryptoUtils::GetLastOpensslError();
        LOG_ERROR_AND_THROW(CryptoErrorException, "Could not generate random data: {}", openssl_error);
    }

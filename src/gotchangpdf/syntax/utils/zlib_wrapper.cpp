...
namespace gotchangpdf
{
		...
		do {
			...
			do {
				...
				case Z_DATA_ERROR:
					...
					if (nullptr == strm.msg)
						throw syntax::ZlibDataErrorException(result->size());
					else
						throw syntax::ZlibDataErrorException(result->size(), std::string(strm.msg));
				case Z_NEED_DICT:
				case Z_MEM_ERROR:
					if (nullptr == strm.msg)
	...
	BufferPtr ZlibWrapper::Inflate(std::istream& input, types::stream_size length)
	{
		...
		catch (syntax::ZlibDataErrorException& ex) {
			auto size = ex.Size();

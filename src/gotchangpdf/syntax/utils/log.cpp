...
namespace gotchangpdf
{
	...
	std::map<const char *, std::shared_ptr<std::mutex>> Logger::m_mutexes;
	...
	{
		return GetScopedWriter(severity, line, scope.c_str(), file, function);
	}
	...
	std::unique_ptr<OutputWriter> Logger::GetScopedWriter(Severity severity, int line, const char * scope, const char * file, const char * function)
	{
		...
		if (nullptr == scope || (0 == strcmp(scope, general_failure_filename))) {
		...
		auto output_stream = std::make_shared<std::ofstream>();
		output_stream->open(scope, std::ios::app);
		if (!output_stream->good()) {

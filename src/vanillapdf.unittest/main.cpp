...
TEST(NameObject, EncodedHexadecimal) {
    ...
    const char NAME_OBJECT_ENCODED_DATA[] = "#01#02#03#FA#FB#FC#FD#FE#FF";
    const char NAME_OBJECT_DECODED_DATA[] = "\x01\x02\x03\xFA\xFB\xFC\xFD\xFE\xFF";

    ASSERT_EQ(NameObject_CreateFromEncodedString(NAME_OBJECT_ENCODED_DATA, &name_ptr), VANILLAPDF_ERROR_SUCCESS);
    ASSERT_NE(name_ptr, nullptr);
    ...
    ASSERT_NE(decoded_buffer_data, nullptr);

    // Verify the data and length returned by Buffer_GetData
    ASSERT_EQ(decoded_buffer_size, strlen(NAME_OBJECT_DECODED_DATA));

    for (uint32_t i = 0; i < decoded_buffer_size; ++i) {
        EXPECT_EQ(decoded_buffer_data[i], NAME_OBJECT_DECODED_DATA[i]);
    }

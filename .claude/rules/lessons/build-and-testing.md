...
- `BIO_reset()` on a **writable** `BIO_s_mem()` clears the buffer (sets length=0), it does NOT rewind to position 0. The fix is simpler: skip `BIO_reset()` entirely and pass the writable BIO directly to `CMS_final()` — it reads from position 0 of the accumulated data without needing a reset or a separate read-only BIO.
